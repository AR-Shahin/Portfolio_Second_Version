...
//INACTIVE SERVICE
if(isset($_GET['inactive']) && isset($_GET['manageservice'])){
    $id =  $_GET['id'];
    $status = new Services();
    $result = $status->inactiveService($id);
    if($result == true){
        header('location: manageservice.php');
    }
}
//ACTIVE SERVICE
if(isset($_GET['active']) && isset($_GET['manageservice'])){
    $id =  $_GET['id'];
    $status = new Services();
    $result = $status->activeService($id);
    if($result == true){
        header('location: manageservice.php');
    }
}
